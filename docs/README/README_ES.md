...
> **Nota**: El comando `pyobsidian-index` requiere los extras `[vector]`. Al usar uvx, debes incluir `[vector]` en la especificación del paquete. Ejecutar `uvx --from py-obsidian-tools pyobsidian-index` sin `[vector]` fallará.
